...
export default function EnglishAnswerSheet() {
  ...
  const sheetName = params.name || 'English Answer Sheet';
  const totalQuestions = parseInt(params.totalQuestions || '45');
  const mcqQuestions = parseInt(params.mcqQuestions || '45');
  const textQuestions = parseInt(params.textQuestions || '0');
  
  console.log('English Answer Sheet Params:', {
    totalQuestions,
    mcqQuestions,
    textQuestions,
  ...
  useEffect(() => {
    ...
    console.log('=== REAL-TIME ENGLISH ANSWER SHEET UPDATE ===');
    ...
    console.log('✅ English Answer Sheet updated with', initialQuestions.length, 'questions');
